...
AuthorID INT PRIMARY KEY, Name VARCHAR(100) NOT NULL, Country VARCHAR(50),DOB DATE
);

-- Q2
CREATE TABLE Categories
( CategoryID INT PRIMARY KEY, CategoryName VARCHAR(100) UNIQUE
);
...
ALTER TABLE OrderDetails ADD Discount DECIMAL(5,2) DEFAULT 0; ALTER TABLE OrderDetails ALTER Discount DROP DEFAULT;
...
SELECT B.BookID, B.Title, SUM(OD.Quantity) AS TotalSold FROM Books B
...
Price DECIMAL(10,2) CHECK (Price > 0),
Stock INT CHECK (Stock >= 0),
PublishedYear YEAR DEFAULT (YEAR(CURDATE())), FOREIGN KEY (AuthorID) REFERENCES Authors(AuthorID),
FOREIGN KEY (CategoryID) REFERENCES Categories(CategoryID) );
...
CREATE TABLE BookReviews ( ReviewID INT PRIMARY KEY, 
BookID INT,
CustomerID INT, 
ReviewText TEXT,
FOREIGN KEY (BookID) REFERENCES Books(BookID),
...
ALTER TABLE BookReviews DROP FOREIGN KEY fk_book; ALTER TABLE BookReviews DROP FOREIGN KEY fk_customer; DROP TABLE BookReviews;



CREATE TABLE Coupons ( CouponID INT PRIMARY KEY,
...
ALTER TABLE Coupons DROP CHECK chk_discount; ALTER TABLE Coupons DROP COLUMN Status;
